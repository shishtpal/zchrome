...
pub const SNAPSHOT_JS = "(function(){var NL='\\n',R={'A':'link','BUTTON':'button','INPUT':function(e){var t=e.type.toLowerCase();if(t==='checkbox')return'checkbox';if(t==='radio')return'radio';if(t==='submit'||t==='button')return'button';return'textbox'},'TEXTAREA':'textbox','SELECT':'combobox','H1':'heading','H2':'heading','H3':'heading','H4':'heading','H5':'heading','H6':'heading','UL':'list','OL':'list','LI':'listitem','TABLE':'table','NAV':'navigation','MAIN':'main','FORM':'form','IMG':'img'};function getRole(e){var r=e.getAttribute('role');if(r)return r.toLowerCase();var m=R[e.tagName];if(typeof m==='function')return m(e);return m||null}function getName(e){var a=e.getAttribute('aria-label');if(a)return a;var t=e.tagName;if(t==='A'||t==='BUTTON'){var x=e.textContent.trim();if(x)return x}if(t==='IMG')return e.getAttribute('alt');if(/^H[1-6]$/.test(t))return e.textContent.trim();return e.getAttribute('title')}function ok(e){if(e.hidden)return false;try{var s=getComputedStyle(e);if(s.display==='none'||s.visibility==='hidden')return false}catch(x){}return['SCRIPT','STYLE','NOSCRIPT','TEMPLATE','SVG'].indexOf(e.tagName)===-1}function build(e,d,m){if(m!==null&&d>m)return'';if(!ok(e))return'';var r='',role=getRole(e);if(role){var name=getName(e),ind='  '.repeat(d);r+=name?ind+'- '+role+' \"'+name.replace(/\"/g,'')+'\"'+NL:ind+'- '+role+NL}var next=role?d+1:d;for(var c of e.children)r+=build(c,next,m);return r}var sel=SEL_ARG,maxD=DEPTH_ARG,root=sel?document.querySelector(sel):document.body;if(sel&&!root)return'(not found)';return build(root,0,maxD)||'(empty)'})()";
