...
/// JavaScript code to extract accessibility tree from the DOM (single line to avoid escaping issues)
pub const SNAPSHOT_JS = "(function(){var NL='\\n',R={'A':'link','BUTTON':'button','INPUT':function(e){var t=(e.type||'text').toLowerCase();if(t==='checkbox')return'checkbox';if(t==='radio')return'radio';if(t==='submit'||t==='button'||t==='reset')return'button';if(t==='hidden')return null;return'textbox'},'TEXTAREA':'textbox','SELECT':'combobox','H1':'heading','H2':'heading','H3':'heading','H4':'heading','H5':'heading','H6':'heading','UL':'list','OL':'list','LI':'listitem','TABLE':'table','NAV':'navigation','MAIN':'main','FORM':'form','IMG':'img'};function getRole(e){var r=e.getAttribute('role');if(r)return r.toLowerCase();var m=R[e.tagName];if(typeof m==='function')return m(e);return m||null}function getName(e){var a=e.getAttribute('aria-label');if(a)return a;var p=e.getAttribute('placeholder');if(p)return p;var t=e.tagName;if(t==='A'||t==='BUTTON'){var x=e.textContent.trim();if(x)return x}if(t==='IMG')return e.getAttribute('alt');if(/^H[1-6]$/.test(t))return e.textContent.trim();if(t==='INPUT'||t==='TEXTAREA'){var id=e.id;if(id){var l=document.querySelector('label[for=\"'+id+'\"]');if(l)return l.textContent.trim()}}return e.getAttribute('title')}function getAttrs(e){var a='';var n=e.getAttribute('name');if(n)a+=' [name=\"'+n+'\"]';var p=e.getAttribute('placeholder');if(p)a+=' [placeholder=\"'+p.replace(/\"/g,'')+'\"]';var i=e.id;if(i)a+=' [id=\"'+i+'\"]';var al=e.getAttribute('alt');if(al)a+=' [alt=\"'+al.replace(/\"/g,'')+'\"]';var t=e.type;if(t&&e.tagName==='INPUT')a+=' [type=\"'+t+'\"]';return a}function ok(e){if(e.hidden)return false;try{var s=getComputedStyle(e);if(s.display==='none'||s.visibility==='hidden')return false}catch(x){}return['SCRIPT','STYLE','NOSCRIPT','TEMPLATE','SVG'].indexOf(e.tagName)===-1}function build(e,d,m){if(m!==null&&d>m)return'';if(!ok(e))return'';var r='',role=getRole(e);if(role){var name=getName(e),ind='  '.repeat(d),at=getAttrs(e);r+=name?ind+'- '+role+' \"'+name.replace(/\"/g,'')+'\"'+at+NL:ind+'- '+role+at+NL}var next=role?d+1:d;for(var c of e.children)r+=build(c,next,m);if(e.shadowRoot)for(var c of e.shadowRoot.children)r+=build(c,next,m);return r}var sel=SEL_ARG,maxD=DEPTH_ARG,root=sel?document.querySelector(sel):document.body;if(sel&&!root)return'(not found)';return build(root,0,maxD)||'(empty)'})()";
